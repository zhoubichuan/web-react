...
export function globalApiMixin(IAMap: any) {
   ...
   * @param {*} name
   */
  IAMap.prototype.findLayer = function (name) {
    const ia = this;
    const { $layers } = ia;

    return $layers.find((item) => item.values_._name === name);
  };
   ...
   * @param {*} condition
   */
  IAMap.prototype.findFeature = function (layerName, condition, values) {
    const ia = this;
    const features = ia.$findLayer(layerName).getSource().getFeatures();

    return features.find((item) => item.values_[condition] === values);
  };
   ...
   * }
   */
  IAMap.prototype.focus = async function (options) {
    const ia = this;
   ...
   * @param {*} name 插件名称
   */
  IAMap.prototype.toggle = function (name) {
    const ia = this;
   ...
   * @returns
   */
  IAMap.prototype.insertLayer = function (name, options) {
    const ia = this;
   ...
   * @returns
   */
  IAMap.prototype.insertDraw = function (name, options) {
    const ia = this;
   ...
   * @returns
   */
  IAMap.prototype.insertPoint = function (name, options) {
    const ia = this;
    const layer = ia.findLayer(name);
   ...
   * }
   */
  IAMap.prototype.insertIcon = function (name, options) {
    const ia = this;
    const layer = ia.findLayer(name);
   ...
   * @returns 线的feature
   */
  IAMap.prototype.insertLine = function (name, options) {
    const ia = this;
    const layer = ia.findLayer(name);
   ...
   */
  IAMap.prototype.insertBuffer = function (name, options) {
    const ia = this;
   ...
   * @param {*} name
   */
  IAMap.prototype.clear = function (name) {
    const ia = this;
    const layer = ia.findLayer(name);
    if (!layer) {
      throw new Error(`名字: ${name}的图层找不到`);
   ...
   * @param {*} name
   */
  IAMap.prototype.remove = function (name, feature) {
    const ia = this;
    const layer = ia.findLayer(name);
    if (!layer) {
      throw new Error(`名字: ${name}的图层找不到`);
   ...
   * @param {*} options
   */
  IAMap.prototype.addIcon = function (options) {
    const ia = this;
    const { $map, $icons } = ia;
   ...
   * 删除地图图标
   */
  IAMap.prototype.removeIcon = function (icon) {
    const ia = this;
    const overlay = ia.$map.getOverlays();
    let idx = 0;

    overlay.array_.forEach((item, index) => {
   ...
   */
  IAMap.prototype.toggleIcon = function (type) {
    const ia = this;
   ...
   * 删除地图feature图标
   */
  IAMap.prototype.removeFeatureIcon = function (icon) {
    const ia = this;
    ...
    if (layer) {
      ...
      const fIndex = features.findIndex((item) => item.ol_uid === icon.ol_uid);
      if (fIndex > -1) {
  ...
  IAMap.prototype.toggleShowFeatureIcon = function (type, iShow) {
    const ia = this;

    ia.$featureIcons.forEach((item) => {
      if (item.$type === type) {
        if (iShow) {
          if (iShow === 1) {
            item.show();
  ...
  IAMap.prototype.douglas = function (points) {
    const ia = this;
    ...
    try {
      return new Promise((res, rej) => {
        ...
        ia.$worker.onmessage = function (event) {
          res(event.data);
        };

        ia.$worker.onerror = function (e) {
          rej(e);
        };
      });
    } catch (error) {}
   ...
   * options: {point: [], text: ''}
   */
  IAMap.prototype.addWaterDropText = function (options) {
    const ia = this;
  ...
  IAMap.prototype.insertPlanRouteLine = async function (
    ...
    if (layer) {
      ...
      arrowLines.forEach((arrowLine, index) => {
        if (index === 0) {
          console.log(
            '%c 🥑 arrowLine: ',
      ...
      // 曲线箭头点
      if (arrow) {
        const curveArrowPointStyle = (fea) => {
          let angle = fea.get('angle');
          return new Style({
            image: new Icon({
        ...
        curveArrowLines.forEach((arrowLine) => {
          let pointFeature = new Feature();
  ...
  */
  IAMap.prototype.dispatchPath = function (list) {
    const ia = this;
    ...
    function lineNumber(point, number) {
      let feature = new Feature({
  ...
  IAMap.prototype.dispatchBuffer = function (points, id, radius, isRemove) {
    const _this = this;
    ...
    if (isRemove) {
      vecterSource.getFeatures().forEach((feature) => {
