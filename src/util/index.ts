 ...
 * Modal.xxx 类型的参数处理
 */
const handleModal = (source, target, key, val) => {
 ...
 */
export const setExportsToComponent = (source, target) => {
...
// 图片路径统一处理
export const imgPath = (path, type?, width?, height?, format?) => {
  if (path) {
...
// 获取文件大小尺寸等信息
export const checkSize = (item, onImageLoaded) => {
